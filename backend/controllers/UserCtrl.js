...
function updateRepoTraffic(repo, traffic) {
  ...
  if(repo.clones.total_count === undefined) {
    repo.clones.total_count = 0
  }
  if(repo.clones.total_uniques === undefined) {
    repo.clones.total_uniques = 0;
  }
    ...
    const userPromises = users.map(async user => {
      ...
      const updateReposPromises = githubRepos.map(async githubRepo => {
        ...
        if(repoEntry === undefined) {
          ...
          if(repoEntry.forks.data.length === 0 || 
            repoEntry.forks.data[repoEntry.forks.data.length - 1].count !== githubRepo.forks_count) {
